      ...
      this.shadowRoot.innerHTML = `
        ...
        <style>
          ...
          *{box-sizing:border-box;margin:0;padding:0;}
          ...
          .lbl{display:flex;justify-content:center;align-items:center;gap:10px;font-size:11.5px;font-weight:700;color:var(--orange);text-transform:uppercase;letter-spacing:.10em;margin-bottom:16px;}
          .lbl::before{content:'';display:block;width:26px;height:2px;background:var(--orange);border-radius:2px;flex-shrink:0}
          ...
          <div class="inner">
            ...
            <div class="cards ob">
              ...
              <div class="card${y?' featured':''}">
                ...
                <ul>${feats.map(f=>`<li>${f}</li>`).join('')}${upsells.map(f=>`<li class="up">${f}</li>`).join('')}</ul>
                <a href="https://buy.stripe.com/aFabJ22gG8sy9Fa1nqeZ200" class="${y?'btn-p':'btn-s'}">Jaarlijks starten</a>
              </div>
            </div>
            <div class="insight ob"><p>"De vraag is niet: kan ik €69 missen? De vraag is: kan ik nog 6 maanden zonder klanten?"</p></div>
            <div class="disc ob"><span>Direct toegang</span><span>Geen verborgen kosten</span><span>Maandelijks opzegbaar</span></div>
