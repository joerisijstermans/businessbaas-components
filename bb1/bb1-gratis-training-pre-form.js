(function () {
  if (customElements.get('bb1-gratis-training-pre-form')) return;
  ...
  class BB1GratisTrainingPreForm extends HTMLElement {
    connectedCallback() { if (!this.id) this.id = 'volgende-stap'; this.style.display = 'block'; this.innerHTML=C.map(n=>{const h=/hero/.test(n)?600:/nav/.test(n)?72:/footer/.test(n)?220:/pricing/.test(n)?580:/faq/.test(n)?480:/cta/.test(n)?320:/notification/.test(n)?0:400;return`<${n} style="display:block;min-height:${h}px"></${n}>`;}).join(''); }
  }
